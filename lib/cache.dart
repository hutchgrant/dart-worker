...
class Cache {
  ...
  Db db;
  Cache(var srvConfig) {
    dbServer = srvConfig["mongo_ip"];
    dbPort = srvConfig["mongo_port"];
    dbName = srvConfig["mongo_table"];
    usrName = srvConfig["mongo_user"];
    usrPass = srvConfig["mongo_pass"];
  }
  ...
  Future<daeObjs> getCoins() {
    ...
    db.open().then((_) {
      ...
      return table.find().forEach((col) => daemons.insert(col["rpcCoin"], col["rpcUser"], col["rpcPass"], col["rpcServer"], col["rpcPort"], col["rpcWallPass"], col["rpcFee"], col["rpcTxFee"], col["rpcRate"], col["rpcMaxConf"], col["rpcEnable"]));
    }).then((_) {
      print('closing db');
      db.close();
  ...
  Future<dbObj> getDB() {
    ...
    db.open().then((_) {
      ...
      return table.find().forEach((col) => mysqlDB.insert(col["ip"], col["name"], col["user"], col["pass"]));
    }).then((_) {
      print('closing db');
      db.close();
  ...
  Future<String> getPref() {
    ...
    db.open().then((_) {
      ...
      return table.find().forEach((col) {
        apikey = col["apikey"];
      });
      //// Reminder to add table + col + select
    }).then((_) {
      print('closing db');
      db.close();
  ...
  void cacheCoin(daeObj dae) {
    ...
    db.open().then((_) {
      ...
      data.add({
        'rpcUser': dae.rpcUser,
        'rpcPass': dae.rpcPass,
        'rpcServer': dae.rpcServer,
        'rpcPort': dae.rpcPort,
        'rpcWallPass': dae.rpcWallPass,
        'rpcCoin': dae.rpcCoin,
        'rpcFee': dae.rpcFee,
        'rpcTxFee': dae.rpcTxFee,
        'rpcEnable': dae.rpcEnable,
        'rpcMaxConf': dae.rpcMaxConf,
        'rpcRate': dae.rpcRate
      });
